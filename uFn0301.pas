...
Var Cheque1 : String;
begin
   if not Verifica_acesso(286) then // permissão para excluir
      Exit;

   if Application.MessageBox(pChar('Exclui o cheque '+ dmFn0301.cdsGridCHEQUE.AsString + '?'), 'Confirmção', MB_YESNO+MB_ICONQUESTION) = mrYes then
   begin
      Cheque1 := 'Excluiu Cheque Nº ' + dmFn0301.cdsGridCHEQUE.AsString + '- Banco ' + dmFn0301.cdsGridBANCO.AsString + '- Emitente - ' + dmFn0301.cdsGridEMITENTE.AsString;
...
begin
   if not Verifica_acesso(284) then  // permissão para incluir
      Exit;
...
begin
   if not Verifica_acesso(285) then // permissão para alterar
      Exit;
