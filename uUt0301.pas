...
begin
   Application.MessageBox('Utilize esta rotina para realizar chamados ' + #13 +
                          'técnicos para a MULTISOFT.' + #13 +
                          'O intuito desta rotina é facilitar os' + #13 +
                          'atendimentos aos nossos clientes,' + #13 +
                          'porém o mesmo deverá conter informações' + #13 +
                          'CLARAS E OBJETIVAS sobre o problema ocorrido' + #13 +
                          'ou as modificações desejadas.','Aviso Importante',MB_ICONINFORMATION) ;
...
begin
   if Application.MessageBox('Confirma a impressão?','Confirmação',MB_ICONQUESTION+MB_YESNO) = mrNo then
      Abort;

   lblData.Caption := DateToStr(DataSystem1);

   case rdgServico.ItemIndex of
     0: lblServico.Caption := 'Solicitação';
     1: lblServico.Caption := 'Alteração';
