...
begin
   ...
   begin
      Application.MessageBox('O valor precisa ser maior que zero.','Atenção', MB_OK + MB_ICONWARNING);
      edtValor.SetFocus;
      exit;      
   end;
   ...
   Daruma_TEF_SetFocus('.:. Receber no Cartão .:.');
   ...
   begin
      ...
      begin
         if Application.MessageBox('Confirma a impressão do recibo?','Confirmação',MB_ICONQUESTION + MB_YESNO) = mrNo then
         begin
         ...
         begin
            ...
            begin
               Application.MessageBox('Não foi possível gerar o arquivo de impressão','Atenção',MB_ICONERROR);
               Abort;
            end;
         end;
      end
