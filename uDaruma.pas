...
Function Daruma_Retorno_Impressora(): Boolean;
  ...
  // Variável geral para tratar o Retorno da Função e tratamento de erros
var
...
//      Daruma_Retorno_Impressora();
    ...
    if iRetorno = 0 then
       Application.MessageBox( 'Erro de comunicação, a Função nao conseguir enviar o comando', 'Daruma32.dll - Erro',
                                MB_IconError + MB_OK);

    If iRetorno = -1 Then
       Application.MessageBox( 'Erro de Execução da Função', 'Daruma32.dll - Erro',
                                MB_IconError + MB_OK);

    if iRetorno = -2  then
       Application.MessageBox( 'Parâmetro inválido passado na função.!', 'Daruma32.dll - Erro',
                                MB_IconError + MB_OK);

    if iRetorno = -3  then
       Application.MessageBox( 'Alíquota não programada no ECF. !', 'Daruma32.dll - Atenção',
                                MB_IconInformation + MB_OK);

    If iRetorno = -4 Then
       Application.MessageBox( 'A Chave ou Valor no Registry não Foi Encontado.!', 'Daruma32.dll - Atenção',
                                MB_IconInformation + MB_OK);

    If iRetorno = -5 Then
       Application.MessageBox( 'Erro ao Abrir a Porta de Comunicação', 'Daruma32.dll - Erro',
                                MB_IconError + MB_OK);

    If iRetorno = -6 Then
       Application.MessageBox( 'Impressora desligada ou cabo de comunicação desconectado.', 'Daruma32.dll - Atenção',
                                MB_IconInformation + MB_OK);

    If iRetorno = -7 Then
       Application.MessageBox( 'Banco não encontrado ou não cadastrado no Registry', 'Daruma32.dll - Atenção',
                                MB_IconInformation + MB_OK);
...
Function Daruma_Retorno_Impressora(): Boolean;
  ...
  Begin
    ...
    begin
          ...
          IF Daruma_iST2 >= 8   Then BEGIN Daruma_iST2 := Daruma_iST2 - 8;   Daruma_MSG := Daruma_MSG + 'Não cabe mais Aliquota!!' + #13 + #10;END;
          IF Daruma_iST2 >= 4   Then BEGIN Daruma_iST2 := Daruma_iST2 - 4;   Daruma_MSG := Daruma_MSG + 'Cancelamento. Não Permitido!!' + #13 + #10;END;
          IF Daruma_iST2 >= 2   Then BEGIN Daruma_iST2 := Daruma_iST2 - 2;   Daruma_MSG := Daruma_MSG + 'CNPJ não Programado!!' + #13 + #10;END;
          IF Daruma_iST2 >= 1   Then BEGIN Daruma_iST2 := Daruma_iST2 - 1;   Daruma_MSG := Daruma_MSG + 'Comando Não Executado!!' + #13 + #10;END;
